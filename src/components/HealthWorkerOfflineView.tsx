...
export const HealthWorkerOfflineView: React.FC = () => {
  ...
  const handleDownloadCarePack = async () => {
    ...
    try {
      ...
      setCarePack(data);
      setLastSyncTime(now);
      setSyncMessage(translate('Smart Care Pack successfully downloaded and cached locally.'));
    } catch (err: any) {
      setSyncMessage(translate('Failed to download Care Pack'));
    } finally {
  ...
  const handleSyncNow = async () => {
    ...
    try {
      ...
      if (queuedActions.length === 0) {
        setSyncMessage(translate('No pending offline changes to synchronize.'));
        setSyncing(false);
        return;
      }
      ...
      setLastSyncTime(now);
      setSyncMessage(
        `${translate('Synchronization complete:')} ${res.syncedCount} / ${res.totalCount} ${translate('actions merged')}`
      );
    } catch (err: any) {
      setSyncMessage(translate('Sync failed'));
    } finally {
      setSyncing(false);
    }
  };
  ...
  const handleClearCache = () => {
    ...
    setCarePack(null);
    setPendingOfflineActionsCount(0);
    setSyncMessage(translate('Offline cache cleared.'));
  };
    ...
    <div className="max-w-4xl mx-auto py-6 px-4 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">
                {translate('Network Status:')} {isOffline ? translate('Offline Field Mode') : translate('Connected to CareGrid Central')}
              </h3>
            </div>
            <p className="text-xs text-slate-500 mt-1">
              {translate('Last Sync:')} {lastSyncTime || translate('Never synchronized on this device')}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <span>{downloading ? translate('Downloading...') : t.offline.downloadPack}</span>
            </button>
            ...
            >
              ...
              <span>{syncing ? translate('Syncing...') : t.offline.syncNow}</span>
            </button>

            <button
              type="button"
              onClick={handleClearCache}
              title={translate('Clear offline storage cache')}
        ...
        <div className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-xs">
          <div>
            <span className="font-bold text-slate-800 block">
              {translate('Pending Offline Changes Queue:')}
            </span>
            <span className="text-slate-500 text-2xs">
              {translate('Actions recorded without network that will sync on reconnect')}
            </span>
          </div>
          <span className="px-2.5 py-1 rounded-full text-xs font-extrabold bg-amber-100 text-amber-900 border border-amber-300">
            {pendingOfflineActionsCount} {translate('Pending')}
          </span>
        </div>
      </div>
      ...
      {carePack ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-800">
              {translate('Active Smart Care Pack (Cached on Device)')}
            </h3>
            <span className="text-2xs font-semibold text-slate-500">
              {translate('Generated:')} {new Date(carePack.generatedAt).toLocaleString()}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3 bg-white rounded-xl border border-slate-200 text-center space-y-1">
              ...
              <div className="text-2xs font-semibold text-slate-500">{translate('Assigned Patients')}</div>
            </div>
            ...
            <div className="p-3 bg-white rounded-xl border border-slate-200 text-center space-y-1">
              ...
              <div className="text-2xs font-semibold text-slate-500">{translate('Pending Referrals')}</div>
            </div>
            ...
            <div className="p-3 bg-white rounded-xl border border-slate-200 text-center space-y-1">
              ...
              <div className="text-2xs font-semibold text-slate-500">{translate('Facility Directory')}</div>
            </div>
            ...
            <div className="p-3 bg-white rounded-xl border border-slate-200 text-center space-y-1">
              ...
              <div className="text-2xs font-semibold text-slate-500">{translate('Essential Medicines')}</div>
            </div>
          </div>
        </div>
      ) : (
        <div className="p-8 text-center bg-white rounded-2xl border border-slate-200 space-y-3">
          ...
          <h3 className="text-sm font-bold text-slate-700">{translate('No Care Pack Downloaded')}</h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            {translate('Click Download Smart Care Pack above to cache patient records, referral tracking, and offline clinical protocols for seamless field operations in low-connectivity areas.')}
          </p>
        </div>
      )}
    </div>
