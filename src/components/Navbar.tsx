    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              ...
              <span className="hidden sm:inline">{t.scenarios.guideTitle}</span>
              ...
              {roleDropdownOpen && (
                <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-lg border border-slate-200 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="px-3 py-1.5 border-b border-slate-100">
                    <p className="text-xs font-semibold text-slate-900">{t.auth.quickPersonaLogin}</p>
                    <p className="text-2xs text-slate-500">{t.auth.switchAccount}</p>
                  </div>
                  {personas.map(p => (
                    <button
                      key={p.role}
                    ...
                    >
                      <div>
                        ...
                        <div className="text-2xs text-slate-500">{t.roles[p.role]}</div>
                      </div>
            ...
            <button
              ...
              title={isOffline ? `${t.common.offline}. ${t.offline.reconnectNow}` : t.common.online}
            ...
            >
              {isOffline ? (
                <>
                  ...
                  <span className="hidden xl:inline font-semibold">{t.common.offline}</span>
                </>
              ) : (
                <>
                  <Wifi className="w-4 h-4 text-emerald-600" />
                  <span className="hidden xl:inline">{t.common.online}</span>
                </>
              )}
            </button>
      ...
      {isOffline && (
        <div className="bg-amber-50 border-t border-b border-amber-200 px-4 py-1.5 text-center text-xs text-amber-900 flex items-center justify-center gap-2">
          ...
          <span>
            {t.offline.offlineModeActiveBanner}
          </span>
          <button
            type="button"
