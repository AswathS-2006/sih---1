      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <div>
          <div className="flex items-center gap-2 text-teal-600 font-bold text-xs uppercase tracking-wider">
            ...
            <span>{translate('CareGrid Evaluator Guide')}</span>
          </div>
          <h2 className="text-xl font-bold text-slate-900 mt-1">
            {translate('Choose a Demo Walkthrough')}
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            {translate('Explore the public healthcare care-coordination platform with pre-configured clinical situations.')}
          </p>
        </div>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              <span className="px-2.5 py-0.5 rounded-full text-2xs font-extrabold uppercase bg-teal-600 text-white">
                {translate('Primary End-to-End Pathway')}
              </span>
          ...
          <div className="flex flex-wrap gap-2 text-2xs text-slate-600 pt-1">
            <span className="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-200 font-medium">
              <Stethoscope className="w-3 h-3 text-teal-600" /> {translate('Digital Triage')}
            </span>
            <span className="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-200 font-medium">
              <ShieldCheck className="w-3 h-3 text-teal-600" /> {translate('Intelligent Routing')}
            </span>
            <span className="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-200 font-medium">
              <Building2 className="w-3 h-3 text-teal-600" /> {translate('Referral Handshake')}
            </span>
            <span className="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-200 font-medium">
              <Radar className="w-3 h-3 text-rose-600" /> {translate('Care-Gap Radar')}
            </span>
          </div>
          ...
          >
            <span>{translate('Launch Scenario 1: Primary Care Pathway')}</span>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              <span className="px-2.5 py-0.5 rounded-full text-2xs font-extrabold uppercase bg-indigo-600 text-white">
                {translate('Specialist Routing & Capacity Logic')}
              </span>
              <h3 className="text-sm font-bold text-slate-900 mt-1.5">
          ...
          <div className="p-2.5 bg-white rounded-xl border border-indigo-100 text-2xs space-y-1">
            <div className="flex justify-between text-rose-800">
              <span>{translate('Facility A (PHC, 4.8 km): No Cardiologist, High Queue, 83% Bed Occupancy')}</span>
              <span className="font-bold">{translate('Score:')} 48</span>
            </div>
            <div className="flex justify-between text-emerald-800 font-semibold">
              <span>{translate('Facility B (District Hospital, 18.2 km): Cardiologist On-Duty, ECG, 60% Capacity')}</span>
              <span className="font-bold">{translate('Score:')} 92 ★</span>
            </div>
          </div>
          ...
          >
            <span>{translate('Launch Scenario 2: Specialist Routing Engine')}</span>
            <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </div>
