...
export const CareJourneyView: React.FC<CareJourneyViewProps> = ({ onNavigateTab }) => {
  ...
  // Load journey whenever selectedPatientId changes
  useEffect(() => {
    ...
    const fetchJourney = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch patient care journey:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleTreatPatientFull = async () => {
    ...
    try {
      ...
      setJourney(updated);
      setSuccessBanner(
        `✓ ${translate('Treatment protocol successfully completed for')} ${updated.patientName}! ${translate('Admitted to')} ${updated.facilityName}.`
      );
    } catch (err) {
  ...
  const handleAdvanceStep = async () => {
    ...
    try {
      ...
      setJourney(updated);
      setSuccessBanner(
        `${translate('Care pathway advanced to:')} ${updated.currentStage}`
      );
    } catch (err) {
  ...
  const handleResetJourney = async () => {
    ...
    try {
      ...
      setJourney(updated);
      setSuccessBanner(
        translate('Care pathway reset to initial doorstep intake state.')
      );
    } catch (err) {
    ...
    <div className="max-w-5xl mx-auto py-6 px-4 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <div className="relative min-w-[280px]">
              <label htmlFor="patient-selector" className="block text-2xs font-bold text-slate-600 uppercase tracking-wider mb-1">
                {translate('Active Patient Pathway:')}
              </label>
                ...
                >
                  {patients.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.name} — {translate(p.district || 'Coimbatore')} (ABHA: {p.abhaId ? p.abhaId.slice(-9) : 'TN'})
                      {user?.patientId === p.id ? ` ★ [${translate('My Record')}]` : ''}
                    </option>
                  ))}
                </select>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/80">
              ...
              <span className="text-2xs text-slate-500">
                {translate(selectedPatient.gender)}, {selectedPatient.age} yrs
              </span>
            </div>

            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/80">
              <span className="text-2xs text-slate-400 block font-medium">
                {translate('ABHA Health ID')}
              </span>
              ...
              <span className="text-3xs text-emerald-700 font-semibold">
                {translate('✓ Verified Ayushman TN')}
              </span>
            </div>

            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/80">
              <span className="text-2xs text-slate-400 block font-medium">
                {translate('Location & District')}
              </span>
              <div className="font-semibold text-slate-800 flex items-center gap-1 mt-0.5">
                <MapPin className="w-3 h-3 text-rose-500 shrink-0" />
                <span>{translate(selectedPatient.district || 'Coimbatore')}, TN</span>
              </div>
            ...
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/80">
              <span className="text-2xs text-slate-400 block font-medium">
                {translate('Assigned Health Worker')}
              </span>
              <div className="font-semibold text-slate-800 flex items-center gap-1 mt-0.5">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-2xs font-extrabold uppercase tracking-widest bg-emerald-500/20 text-emerald-300 border border-emerald-400/30">
              ...
              <span>{translate('Clinical Treatment Action Suite')}</span>
            </div>
            <h2 className="text-lg font-bold text-white">
              {translate('Treatment Pathway for')} {selectedPatient?.name || translate('Patient')}
            </h2>
            <p className="text-xs text-teal-200 max-w-xl">
              {translate('Instantly advance through hospital referral, bed assignment, lab diagnostics, specialist consultation, and prescribed medication regimen.')}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            >
              ...
              <span>
                {translate('Get Me Treated (Complete Treatment)')}
              </span>
            </button>
            ...
            >
              ...
              <span>{translate('Advance 1 Step')}</span>
            </button>
            ...
            <button
              ...
              disabled={actionLoading}
              title={translate('Reset journey to initial registration')}
        ...
        <div className="mt-4 pt-4 border-t border-teal-700/60">
          <div className="flex items-center justify-between text-2xs text-teal-200 mb-1.5">
            <span className="font-semibold">
              {translate('Care Pathway Progress:')} {translate('Step')} {completedStepsCount} {translate('of')} {totalStepsCount} {translate('Completed')}
            </span>
      ...
      {successBanner && (
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-900 text-xs flex items-start gap-2.5 shadow-xs animate-in fade-in">
          ...
          <div className="flex-1">
            <p className="font-bold">{translate('Care Pathway Status Updated')}</p>
      ...
      {loading ? (
        <div className="p-12 text-center text-xs text-slate-500 bg-white rounded-2xl border border-slate-200 flex flex-col items-center justify-center gap-3">
          ...
          <span>{translate('Loading patient care pathway and clinical milestones...')}</span>
        </div>
      ) : journey ? (
        <div className="space-y-6">
            ...
            <div className="relative z-10 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="px-2.5 py-0.5 text-2xs font-bold rounded-full bg-teal-700/80 text-teal-100 border border-teal-600">
                  Stage: {translate(journey.currentStage)}
                </span>
              </div>

              <h2 className="text-base sm:text-lg font-bold leading-relaxed">{translate(journey.nextBestAction)}</h2>
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  <span>
                    {completionPercentage === 100
                      ? translate('Treated & Discharged (0 mins)')
                      : translate('Est. Wait: ~12 mins')}
                  </span>
                </span>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 flex-wrap gap-2">
              <div>
                <h3 className="text-sm font-bold text-slate-900">{translate(journey.title)}</h3>
                <p className="text-2xs text-slate-500 mt-0.5">
                  {translate('Current Clinical Stage:')}{' '}
                  <span className="font-semibold text-teal-700">{translate(journey.currentStage)}</span>
                </p>
              </div>
              <span className="px-3 py-1 text-xs font-bold rounded-full bg-teal-50 text-teal-800 border border-teal-200 flex items-center gap-1.5">
                ...
                <span>Overall: {translate(journey.overallStatus)}</span>
              </span>
            </div>
                      ...
                      <div className="space-y-1 flex-1">
                        ...
                        {step.actionRequired && (
                          <div className="flex items-start gap-1.5 text-xs text-teal-950 font-semibold bg-teal-100/70 p-2 rounded-lg border border-teal-300/80 mt-1.5">
                            ...
                            <span>Action: {translate(step.actionRequired)}</span>
                          </div>
                        )}
                      </div>
