...
export const CareGapRadarView: React.FC = () => {
  ...
  const handleResolveGap = async (status: CareGapStatus) => {
    ...
    try {
      ...
      console.error('Error updating care gap:', err);
    }
  };
    ...
    <div className="max-w-6xl mx-auto py-6 px-4 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-800">
          ...
          <span>
            {gaps.filter(g => g.status !== 'RESOLVED').length} {translate('Active Unresolved Gaps')}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-slate-700">{translate('Filter Severity:')}</span>
        ...
        <div className="flex items-center gap-2">
          <span className="font-semibold text-slate-700">{translate('Status:')}</span>
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="p-8 text-center text-xs text-slate-500 bg-white rounded-xl border border-slate-200">
            {translate('Scanning for overdue follow-ups, delayed referrals, and missing diagnostic tests...')}
          </div>
        ) : filtered.length === 0 ? (
          <div className="p-8 text-center text-xs text-slate-500 bg-white rounded-xl border border-slate-200">
            {translate('No care gaps found matching the active filters.')}
          </div>
        ) : (
          filtered.map(gap => {
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {translate(gap.severity)}
                      </span>
                      <span className="text-2xs font-bold uppercase tracking-wider text-slate-500">
                        {translate(gap.gapType.replace(/_/g, ' '))}
                      </span>
                    </div>
                    <h3 className="text-sm font-bold text-slate-900 mt-1">
                      {gap.patientName} — {translate(gap.resolutionNotes || '')}
                    </h3>
                  </div>
                  ...
                  <div className="text-right shrink-0 space-y-1">
                    ...
                    >
                      {translate(gap.status)}
                    </span>
                    <span className="block text-2xs text-slate-500">
                      {translate('Due:')} {new Date(gap.dueDate).toLocaleDateString()}
                    </span>
                  </div>
                </div>
                  ...
                  {!isResolved ? (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        {translate('Resolve Gap')}
                      </button>
                    </div>
                  ) : (
                    <span className="text-2xs text-emerald-800 font-medium">
                      ✓ {translate('Resolved:')} {translate(gap.actionTaken || '')}
                    </span>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-200 space-y-4">
            <h3 className="text-sm font-bold text-slate-900">
              {translate('Resolve Care Gap:')} {activeGap.patientName}
            </h3>
            <p className="text-xs text-slate-500">
              {translate('Record the clinical or community outreach intervention performed to close this care gap.')}
            </p>

            <div>
              <label className="text-xs font-semibold text-slate-700 block mb-1">
                {translate('Intervention / Action Taken')}
              </label>
              <input
                type="text"
                value={resolveAction}
            ...
            <div>
              <label className="text-xs font-semibold text-slate-700 block mb-1">
                {translate('Clinical Resolution Details & Follow-up Plan')}
              </label>
              <textarea
                rows={3}
                value={resolveNotes}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {translate('Confirm & Mark Resolved')}
              </button>
            </div>
          </div>
        </div>
