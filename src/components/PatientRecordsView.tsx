...
export const PatientRecordsView: React.FC = () => {
  const { user, t, translate, language } = useAuth();
  ...
  const fetchRecord = async () => {
    ...
    try {
      ...
      const c = await api.getConsents(patientId);
      setPatient(p);
      setConsents(c);
    } catch (err) {
  ...
  const handleToggleConsent = async (consentId: string, currentStatus: string) => {
    ...
    try {
      ...
      setFeedbackMessage(
        `${translate('Consent status updated to')} ${translate(nextStatus)}`
      );
      setTimeout(() => setFeedbackMessage(null), 3000);
    } catch (err: any) {
      setFeedbackMessage(translate('Failed to update consent'));
    }
  };
  ...
  const handleRequestTeleconsult = async () => {
    ...
    try {
      ...
      setFeedbackMessage(translate('eSanjeevani teleconsultation session established'));
      setTimeout(() => setFeedbackMessage(null), 4000);
    } catch (err: any) {
      setFeedbackMessage(translate('Teleconsultation error'));
    } finally {
    ...
    <div className="max-w-5xl mx-auto py-6 px-4 space-y-6">
      ...
      {loading ? (
        <div className="p-8 text-center text-xs text-slate-500 bg-white rounded-xl border border-slate-200">
          {translate('Loading longitudinal patient records...')}
        </div>
      ) : patient ? (
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1 text-xs text-slate-600">
                    <span>{patient.age} Yrs / {translate(patient.gender)}</span>
                    <span>•</span>
                    <span className="font-semibold text-rose-600">Blood: {patient.bloodGroup}</span>
                  </div>
                  <p className="text-2xs text-slate-500 mt-0.5">
                    {patient.addressVillage}, {translate(patient.district)}, {translate(patient.state)}
                  </p>
                </div>
                ...
                <div className="text-right">
                  ...
                  <span className="text-3xs text-slate-400 block mt-0.5">{translate('ABDM Verified')}</span>
                </div>
              </div>
              ...
              {patient.allergies && patient.allergies.length > 0 && (
                <div className="p-3 bg-rose-50 border border-rose-300 rounded-xl flex items-center gap-2 text-xs text-rose-900">
                  ...
                  <div>
                    <strong>{translate('Severe Drug Allergy Alert:')}</strong>{' '}
                    <span className="font-bold underline">{patient.allergies.map(a => translate(a)).join(', ')}</span> ({translate('Risk of anaphylaxis')})
                  </div>
                </div>
              )}
              ...
              <div>
                <span className="text-2xs font-bold text-slate-500 uppercase tracking-wider block mb-1">
                  {translate('Chronic Diagnoses & Conditions:')}
                </span>
              ...
              <div>
                <span className="text-2xs font-bold text-slate-500 uppercase tracking-wider block mb-1">
                  {translate('Active Prescribed Medications:')}
                </span>
                <div className="space-y-1">
                  {patient.medications.map(m => (
                    ...
                    >
                      ...
                      <span className="font-medium">{translate(m)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 text-xs flex items-center justify-between text-slate-600">
                <span className="flex items-center gap-1.5">
                  ...
                  {translate('Emergency Contact:')} <strong>{patient.emergencyContactName}</strong>
                </span>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
                    ...
                    {t.records.teleconsultTitle}
                  </h3>
                  <p className="text-2xs text-slate-500">
                    {translate('Government telemedicine adapter for remote rural doctor consultations')}
                  </p>
                </div>
                <button
                  type="button"
                ...
                >
                  {teleconsultLoading ? translate('Connecting...') : t.records.requestTeleconsult}
                </button>
              </div>
              ...
              {teleconsultResult && (
                <div className="p-3 bg-teal-50 border border-teal-200 rounded-xl space-y-2 text-xs">
                  ...
                  <p className="text-2xs text-teal-900 italic">
                    {translate(teleconsultResult.regulatoryNote || '')}
                  </p>
                  <div className="pt-1">
                    <span className="inline-block px-3 py-1 rounded-md text-xs font-bold bg-teal-700 text-white">
                      {translate('Enter Tele-Consultation Bay →')}
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
              <div className="border-b border-slate-100 pb-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <h3 className="text-sm font-bold text-slate-900">{t.records.abdmConsent}</h3>
                </div>
                <p className="text-2xs text-slate-500 mt-0.5">
                  {translate('Patient controls which hospitals and specialists can view their diagnostic reports.')}
                </p>
              </div>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-2xs text-slate-500 mt-0.5">{translate(consent.purpose)}</p>
                        </div>
                        <span
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-200 text-2xs">
                        <span className="text-slate-500">{translate('Scope:')} {consent.dataScope}</span>
                        <button
                          type="button"
                        ...
                        >
                          {isGranted ? translate('Revoke Consent') : translate('Grant Consent')}
                        </button>
                      </div>
                    </div>
                  );
