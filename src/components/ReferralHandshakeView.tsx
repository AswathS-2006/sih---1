    ...
    <div className="max-w-6xl mx-auto py-6 px-4 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          ...
          <span>{translate('New Referral Handshake')}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-3">
          <div className="flex items-center justify-between text-xs font-semibold text-slate-600 px-1">
            <span>{translate('Referrals In Handshake Pipeline')}</span>
            <span>{referrals.length} {translate('active')}</span>
          </div>

          {loading ? (
            <div className="p-8 text-center text-xs text-slate-500 bg-white rounded-xl border border-slate-200">
              {translate('Loading referral pipeline...')}
            </div>
          ) : (
            referrals.map(r => {
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <h4 className="text-sm font-bold text-slate-900 mt-1">
                        {translate('To:')} {r.targetFacilityName}
                      </h4>
                      <p className="text-2xs text-slate-500">{translate('By:')} {r.referringWorkerName}</p>
                    </div>
                    ...
                    <div className="text-right space-y-1">
                      ...
                      >
                        {translate(r.priorityLevel)}
                      </span>
                    </div>
                  </div>

                  <p className="text-xs text-slate-600 line-clamp-1 italic">
                    "{translate(r.reasonForReferral)}"
                  </p>
                </div>
              );
            })
        ...
        <div className="lg:col-span-7">
          {selectedReferral ? (
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-5">
              ...
              <div className="flex items-start justify-between border-b border-slate-100 pb-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {translate(selectedReferral.status)}
                    </span>
                  </div>
                  ...
                  <p className="text-xs text-slate-500">
                    {translate('Initiated by')} {selectedReferral.referringWorkerName} {translate('on')}{' '}
              ...
              <div>
                <span className="text-2xs font-bold text-slate-500 uppercase tracking-wider block mb-2">
                  {translate('Handshake Lifecycle Status:')}
                </span>
                      ...
                      return (
                        <div key={stepName} className="flex items-center gap-1 shrink-0">
                          ...
                          <span className={isDone ? 'text-teal-900 font-bold' : 'text-slate-400'}>
                            {translate(stepName.replace(/_/g, ' '))}
                          </span>
                        </div>
                      );
                    }
              ...
              {selectedReferral.status === 'REJECTED' && (
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl space-y-2">
                  <div className="flex items-center gap-2 text-xs font-bold text-rose-900">
                    ...
                    <span>{translate('Referral Declined by Receiving Facility')}</span>
                  </div>
                  <p className="text-xs text-rose-800">
                    {translate('Reason:')} {translate(selectedReferral.rejectionReason || '')}
                  </p>
                    ...
                    <div className="p-2 bg-white rounded-lg border border-rose-200 flex items-center justify-between">
                      <div>
                        <span className="text-2xs font-bold text-slate-700 block">
                          {translate('CareGrid Alternative Routing Recommendation:')}
                        </span>
              ...
              <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                <span className="text-2xs font-bold text-slate-700 uppercase tracking-wider block">
                  {translate('Clinical Summary & Vital Signs:')}
                </span>
                <p className="text-xs text-slate-800 whitespace-pre-line leading-relaxed">
                  {translate(selectedReferral.clinicalSummary)}
                </p>
              </div>
              ...
              <div className="p-4 bg-teal-50/70 border border-teal-200 rounded-xl space-y-3">
                <span className="text-xs font-bold text-teal-900 block">
                  {translate('Receiving Facility & Clinician Handshake Actions:')}
                </span>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedReferral.status === 'SENT' || selectedReferral.status === 'UNDER_REVIEW' ? (
                    <>
                      ...
                      >
                        ...
                        <span>{translate('Accept Patient & Bed')}</span>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <span>{translate('Decline / Re-route')}</span>
                      </button>
                    </>
                  ) : null}
                  ...
                  {selectedReferral.status === 'ACCEPTED' ? (
                    ...
                    >
                      ...
                      <span>{translate('Issue Priority Token')}</span>
                    </button>
                  ) : null}
                  ...
                  {selectedReferral.status === 'APPOINTMENT_ASSIGNED' ? (
                    ...
                    >
                      ...
                      <span>{translate('Mark Patient Arrived')}</span>
                    </button>
                  ) : null}
                  ...
                  {selectedReferral.status === 'PATIENT_ARRIVED' ? (
                    ...
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" />
                      <span>{translate('Complete Consultation')}</span>
                    </button>
                  ) : null}
                  ...
                  {selectedReferral.status === 'CONSULTATION_COMPLETED' ? (
                    ...
                    >
                      ...
                      <span>{translate('Close Handshake Pathway')}</span>
                    </button>
                  ) : null}
                </div>
              </div>
              ...
              <div>
                <span className="text-2xs font-bold text-slate-500 uppercase tracking-wider block mb-2">
                  {translate('Handshake Audit Trail:')}
                </span>
                <div className="space-y-2 border-l-2 border-slate-200 pl-3">
                  {selectedReferral.statusHistory.map((h, i) => (
                    <div key={i} className="text-2xs space-y-0.5">
                      <div className="flex items-center justify-between text-slate-700">
                        <span className="font-bold">{translate(h.status)}</span>
                        <span className="text-slate-400">
                          {new Date(h.timestamp).toLocaleTimeString()}
                        </span>
                      </div>
                      <p className="text-slate-600">{translate(h.notes || '')}</p>
                      <span className="text-3xs text-slate-400 italic">{translate('By:')} {h.updatedBy}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="p-8 text-center text-xs text-slate-500 bg-slate-50 rounded-2xl border border-slate-200">
              {translate('Select a referral on the left to review handshake details and take actions.')}
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-900">
                {translate('Initiate Referral Handshake')}
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateReferral} className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-slate-700 block mb-1">
                  {translate('Target Receiving Facility')}
                </label>
                <select
              ...
              <div>
                <label className="text-xs font-semibold text-slate-700 block mb-1">
                  {translate('Clinical Summary & Initial Measures Given')}
                </label>
                <textarea
                  rows={3}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-200 space-y-4">
            <h3 className="text-sm font-bold text-slate-900">
              {translate('Decline Referral & Re-route')}
            </h3>
            <p className="text-xs text-slate-500">
              {translate('Specify the clinical reason for non-acceptance. CareGrid will automatically propose the closest alternative facility.')}
            </p>
            <textarea
              rows={3}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {translate('Confirm Decline & Re-route')}
              </button>
            </div>
          </div>
        </div>
