...
export function recommendFacilities(request: RoutingRequest): ScoredFacility[] {
  ...
  for (const fac of facilities) {
    ...
    const isBigHospital =
      fac.type === 'Specialist Medical College' ||
      fac.type === 'Tertiary Hospital' ||
    ...
    if (request.urgency === 'RED') {
      // High Risk: Prioritize Big Hospitals with 24x7 Emergency, ICU, and Trauma capabilities
      if (isBigHospital && fac.emergencyCapability && fac.icuBeds >= 20) {
        capabilityMatch = 20;
        reasons.push('✓ Top Recommendation for High Risk: Big Hospital with 24x7 Emergency Trauma, ICU Resuscitation & Specialists');
      } else if (fac.emergencyCapability && fac.icuBeds > 0) {
        capabilityMatch = 16;
        reasons.push('✓ Big/Secondary Hospital with Emergency Room & ICU beds');
      } else if (fac.emergencyCapability) {
        capabilityMatch = 10;
        reasons.push('✓ Basic 24x7 Emergency Room available');
      } else {
        // Local clinic or PHC with no emergency/ICU beds penalized heavily for RED urgency
        capabilityMatch = 0;
        reasons.push('⚠ Critical High-Risk Alert: Local Clinic lacks 24x7 emergency resuscitation, ICU, and surgical facilities. Big hospital emergency referral required!');
      }
    } else if (request.urgency === 'ORANGE') {
      if (fac.type === 'CHC' || fac.type === 'Sub-District Hospital' || fac.type === 'District Hospital') {
        capabilityMatch = 20;
        reasons.push('✓ Full secondary care inpatient and observation capabilities');
      } else if (fac.emergencyCapability) {
        capabilityMatch = 16;
        reasons.push('✓ Inpatient hospital care with emergency backup');
      } else if (isClinicOrPHC) {
        capabilityMatch = 10;
        reasons.push('⚠ Local Clinic can provide basic stabilization prior to secondary transfer');
      } else {
        capabilityMatch = 14;
      }
    } else {
      // GREEN (Low Risk Problems): Prioritize Local Clinics and PHCs over crowded big hospitals
      if (isClinicOrPHC) {
        capabilityMatch = 20;
        reasons.push('✓ Top Recommendation for Low Risk: Local Primary Care Clinic / Ayushman Arogya Mandir (fast OPD & vitals check without big hospital congestion)');
      } else if (fac.type === 'CHC' || fac.type === 'Sub-District Hospital') {
        capabilityMatch = 14;
        reasons.push('✓ Community Health Centre suitable for routine outpatient review');
      } else {
        // Big tertiary / district hospitals de-prioritized for low-risk routine symptoms
        capabilityMatch = 5;
        reasons.push('⚠ Big Hospital de-prioritized for routine low-risk symptoms to prevent emergency department crowding and long transit');
      }
    }
